...
export function FilterSidebar({ amenities }: FilterSidebarProps) {
  ...
  const applyFilters = () => {
    ...
    if (filters.minSqft) {
      params.set("minSqft", filters.minSqft)
    }

    if (filters.maxSqft) {
      params.set("maxSqft", filters.maxSqft)
    }

    if (filters.minYear) {
      params.set("minYear", filters.minYear)
    }

    if (filters.maxYear) {
      params.set("maxYear", filters.maxYear)
    }

    if (filters.minLotSize) {
      params.set("minLotSize", filters.minLotSize)
    }

    if (filters.maxLotSize) {
      params.set("maxLotSize", filters.maxLotSize)
    }
    ...
    if (filters.openHouse) {
      params.set("openHouse", "true")
    }

    if (filters.priceReduced) {
      params.set("priceReduced", "true")
    }
